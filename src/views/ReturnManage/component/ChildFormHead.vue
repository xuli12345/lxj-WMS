    ...
    >
      <template v-for="(item, index) in tableHead">
        ...
        >
          ...
          <el-input
            v-else-if="
              (item.fColumn == 'fQty' && item.fDataType == 'decimal') ||
...
import { getTableHeadData, getTableBodyData, getOrderNo,getTableSelData } from "@/api/index";

export default {
  props: ["fTableViewHead", "addItem", "selectArr", "alertArr", "StateObj"],
  data() {
    return {
      //表单域标签的位置
      labelPosition: "right",
  ...
  methods: {
    ...
    getPartsValue() {
      ...
      if (this.ruleForm.fQty) {
        fQty = this.ruleForm.fQty;
      }
      let value = Number(fPrice) * parseInt(fQty);
      this.$set(this.ruleForm, "fRtnAmount", value);
    },
    //获取form表单数据
    ...
    selectFunction(v) {
      ...
      this.selectArr.forEach(element => {
        // console.log(element.fName,v,"eeiie")
        if (element.fName == v) {
          cc = true;
        }
      });
